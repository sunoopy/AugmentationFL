...
class ZSDGFedAvgHFL:
    def __init__(self, num_clients, model_fn, num_rounds, local_epochs, use_zsdg=True, zsdg_samples_per_client=100):
        ...
        self.zsdg_samples_per_client = zsdg_samples_per_client

    def zsdg(self):
        if not self.use_zsdg:
            return None, None

        num_classes = self.global_model.output_shape[-1]
        samples_per_class = self.zsdg_samples_per_client // num_classes

        fake_data_list = []
        fake_labels_list = []

        for class_label in range(num_classes):
            class_data = tf.random.normal(shape=(samples_per_class,) + self.global_model.input_shape[1:])
        ...
        for _ in range(self.local_epochs):
            with tf.GradientTape() as tape:
                predictions = self.global_model(fake_data, training=True)
    ...
    def client_update(self, client_model, client_data, fake_data=None, fake_labels=None):
        ...
        if self.use_zsdg and fake_data is not None and fake_labels is not None:
            train_data = np.concatenate([train_data, fake_data])
    ...
    def train(self, client_data):
        # Perform ZSDG once before training
        if self.use_zsdg:
            fake_data, fake_labels = self.zsdg()
            fake_data_per_client = fake_data.shape[0] // self.num_clients
        else:
        ...
        for round in range(self.num_rounds):
            ...
            for i, (client_x, client_y) in enumerate(client_data):
                ...
                if self.use_zsdg:
                    start_idx = i * fake_data_per_client
                    end_idx = (i + 1) * fake_data_per_client
                    client_fake_data = fake_data[start_idx:end_idx]
                    client_fake_labels = fake_labels[start_idx:end_idx]
                else:
                    client_fake_data, client_fake_labels = None, None

                updated_weights = self.client_update(client_model, (client_x, client_y), client_fake_data, client_fake_labels)
...
if __name__ == "__main__":
    ...
    federated_learning = ZSDGFedAvgHFL(num_clients=num_clients, model_fn=create_model, 
                                       ...
                                       use_zsdg=use_zsdg, zsdg_samples_per_client=zsdg_samples_per_client)
